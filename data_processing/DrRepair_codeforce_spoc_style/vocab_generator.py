...
def generate_vocab(path, validation_users):
    tok_list = list()
    data_path = os.path.join(path, 'err-data-compiler--auto-corrupt--codeforce--spoc-style/')
    ...
    with open('data_processing/DrRepair_codeforce_spoc_style/target_vocab.json', 'w') as f:
        json.dump(tok_dict, f, indent=4)
    with open('data_processing/DrRepair_codeforce_spoc_style/target_vocab_reverse.json', 'w') as f:
        json.dump(tok_dict_re, f, indent=4)
    print("Exceptions in mutate() call: {}".format(exceptions_in_mutate_call))

if __name__ == '__main__':
    path = os.path.join('data_processing', 'DrRepair_codeforce_spoc_style')
